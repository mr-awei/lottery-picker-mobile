...
export const CHANGELOG = [
  {
    version: '1.10.0',
    date: '2026-09-09',
    title: '双授权协议 v1.0 接入 · 应用内开源协议展示',
    items: [
      '【协议升级】项目开源协议由 MIT 正式变更为「彩票选号器手机版 双授权协议 v1.0」（Lottery Picker Mobile Dual License v1.0），根目录新增 LICENSE 文件，含中英双语完整条款',
      '【非商业免费】个人用户、非盈利组织、教育机构可免费使用、复制、修改、分发本软件；任何商业用途（集成到收费产品 / 提供收费服务 / 企业内部商业项目）必须事先联系作者购买商业授权',
      '【恶意程序三层禁止】防篡改注入（严禁在本软件中植入恶意代码）/ 防植入恶意程序（严禁将本软件嵌入恶意软件）/ 防用于恶意程序（严禁与恶意程序结合使用），免费授权版与商业授权版均必须遵守，违反者保留起诉权利',
      '【应用内入口】设置页 → 关于软件 卡片底部新增「开源协议」按钮，点击弹出全屏对话框，可滚动展示完整中英双语协议全文（等宽字体，最大高度 60vh）',
      '【联系邮箱】关于软件区新增「联系邮箱：new_mr_awei@163.com」按钮，点击自动复制邮箱到剪贴板（含 execCommand 降级方案 + ElMessage 提示）；协议弹窗底部同样提供复制邮箱入口',
      '【README & package.json】README 顶部版本行与底部 License 章节重写为双授权协议声明（含非商业免费 / 商业授权 / 恶意程序禁止 / 违约追责四要点）；package.json license 字段由 MIT 改为 SEE LICENSE IN LICENSE'
    ]
  },
  {
    version: '1.9.10',
